...
def enable(
    ...
    Clients should not assume that the debugging has been
    enabled until the result for this command is received.
    :param max_scripts_cache_size: **(EXPERIMENTAL)** *(Optional)*
    The maximum size in bytes of collected scripts
    (not referenced by other heap objects) the debugger can hold.
    Puts no limit if parameter is omitted.
...
def evaluate_on_call_frame(
    ...
    T_JSON_DICT,
    typing.Tuple[
        runtime.RemoteObject, typing.Optional[runtime.ExceptionDetails]
    ],
]:
    """
    ...
    :param object_group: *(Optional)*
     String object group name to put result into
     (allows releasing resulting object handles using ``releaseObjectGroup``).
    :param include_command_line_api: *(Optional)*
     Specifies whether command line API should be available
    to the evaluated expression, defaults to false.
    ...
    :param throw_on_side_effect: *(Optional)*
...
def get_possible_breakpoints(
    ...
    :param restrict_to_function: *(Optional)*
...
def disassemble_wasm_module(
    ...
    T_JSON_DICT,
    typing.Tuple[
        typing.Optional[str], int, typing.List[int], WasmDisassemblyChunk
    ],
]:
    """
    **EXPERIMENTAL**
    ...
    :returns: A tuple with the following items:
        0. **streamId** - *(Optional)*
          ...
          of disassembly can be read successively.
        1. **totalNumberOfLines** -
          The total number of lines in the disassembly text.
        2. **functionBodyOffsets** -
          The offsets of all function bodies,
          in the format [start1, end1, start2, end2, ...] where all ends
          are exclusive.
...
def resume(
    ...
    :param terminate_on_resume: *(Optional)*
    Set to true to terminate execution upon resuming execution.
    In contrast to Runtime.terminateExecution,
    this will allows to execute further JavaScript (i.e. via evaluation)
    until execution of the paused code is actually resumed,
    at which point termination is triggered.
...
def set_async_call_stack_depth(
    ...
    Setting to `0` will disable collecting async call stacks (default).
    """
    params: T_JSON_DICT = dict()
...
def set_blackbox_patterns(
    ...
    Forces backend to skip stepping/pausing in
    scripts with url matching one of the patterns.
...
def set_breakpoint(
    ...
    T_JSON_DICT, T_JSON_DICT, typing.Tuple[BreakpointId, Location]
]:
    """
...
def set_breakpoint_by_url(
    ...
    """
    Sets JavaScript breakpoint at given location specified either by URL
    or URL regex. Once this command is issued, all existing parsed scripts
    ...
    ``breakpointResolved`` events issued.
...
def set_pause_on_exceptions(
    ...
    or caught exceptions, no exceptions.
...
def set_script_source(
    ...
    :param dry_run: *(Optional)*
     ...
     Dry run may be used to get result description
     without actually modifying the code.
    :param allow_top_frame_editing: **(EXPERIMENTAL)** *(Optional)*
     If true, then ``scriptSource`` is allowed to change the function
     on top of the stack as long as the top-most stack frame is the
     only activation of that function.
    :returns: A tuple with the following items:
        0. **callFrames** - *(Optional)*
          New stack trace in case editing has happened while VM was stopped.
        1. **stackChanged** - *(Optional)*
          Whether current call stack was modified after applying the changes.
        2. **asyncStackTrace** - *(Optional)* Async stack trace, if any.
        3. **asyncStackTraceId** - *(Optional)* Async stack trace, if any.
        4. **status** - Whether the operation was successful or not.
          Only `` Ok`` denotes a successful live edit
          while the other enum variants denote why the live edit failed.
        5. **exceptionDetails** - *(Optional)*
          Exception details if any.
          Only present when ``status`` is `` CompileError`.
    """
    params: T_JSON_DICT = dict()
    params["scriptId"] = script_id.to_json()
...
def set_skip_all_pauses(
    ...
    """
    Makes page not interrupt on any pauses
    (breakpoint, exception, dom exception etc).
...
def set_variable_value(
    ...
    :param scope_number: 0-based number of scope as was listed in scope chain.
     Only 'local', 'closure' and 'catch' scope types are allowed.
     Other scopes could be manipulated manually.
...
def step_into(
    ...
    :param break_on_async_call: **(EXPERIMENTAL)** *(Optional)*
     Debugger will pause on the execution of the first async task
     which was scheduled before next pause.
    :param skip_list: **(EXPERIMENTAL)** *(Optional)*
...
class ScriptFailedToParse:
    ...
    #: JavaScript top stack frame of where the script parsed event
    #: was triggered if available.
    stack_trace: typing.Optional[runtime.StackTrace]
    #: If the scriptLanguage is WebAssembly,
    #: the code section offset in the module.
...
class ScriptParsed:
    ...
    #: frameId: string}
    execution_context_aux_data: typing.Optional[dict]
    #: True, if this script is generated as a result
    #: of the live edit operation.
    ...
    script_language: typing.Optional[ScriptLanguage]
    #: If the scriptLanguage is WebASsembly,
    #: the source of debug symbols for the module.
