...
class Node:
    ...
    #: This property used to return the imported document
    #: for the HTMLImport links.
    ...
    def to_json(self) -> T_JSON_DICT:
        ...
        if self.pseudo_elements is not None:
            json["pseudoElements"] = [
                i.to_json() for i in self.pseudo_elements
            ]
        ...
        if self.distributed_nodes is not None:
            json["distributedNodes"] = [
                i.to_json() for i in self.distributed_nodes
            ]
...
def collect_class_names_from_subtree(
    ...
    """
    Collects class names for the node with given id and all of its child nodes.
    **EXPERIMENTAL**
...
def describe_node(
    ...
    :param depth: *(Optional)* The maximum depth at which children should be
     retrieved, defaults to 1.
     Use -1 for the entire subtree or provide an integer larger than 0.
    :param pierce: *(Optional)*
...
def scroll_into_view_if_needed(
    ...
    """
    Scrolls the specified rect of the given node into view
    if not already visible.
    Note: exactly one between nodeId,
    backendNodeId and objectId should be passed to identify the node.
    :param node_id: *(Optional)* Identifier of the node.
    :param backend_node_id: *(Optional)* Identifier of the backend node.
    :param object_id: *(Optional)* JavaScript object id of the node wrapper.
    :param rect: *(Optional)*
     The rect to be scrolled into view, relative to the node's border box,
     in CSS pixels. When omitted, center of the node will be used,
     similar to Element.scrollIntoView.
    """
    params: T_JSON_DICT = dict()
    if node_id is not None:
        params["nodeId"] = node_id.to_json()
...
def get_flattened_document(
    ...
    """
    Returns the root DOM node (and optionally the subtree) to the caller.
    Deprecated, as it is not designed to work well with
    the rest of the DOM agent.
    Use DOMSnapshot.captureSnapshot instead.
    .. deprecated:: 1.3
    :param depth: *(Optional)*
     The maximum depth at which children should be retrieved, defaults to 1.
     Use -1 for the entire subtree or provide an integer larger than 0.
    :param pierce: *(Optional)* Whether or not iframes and shadow roots
     should be traversed when returning the subtree (default is false).
    :returns: Resulting node.
    """
    params: T_JSON_DICT = dict()
    if depth is not None:
...
def get_nodes_for_subtree_by_style(
    ...
    **EXPERIMENTAL**
    ...
    :param computed_styles: The style to filter nodes by
     (includes nodes if any of properties matches).
    :param pierce: *(Optional)*
...
def request_child_nodes(
    ...
    :param depth: *(Optional)*
     ...
     Use -1 for the entire subtree or provide an integer larger than 0.
    :param pierce: *(Optional)* Whether or not iframes and shadow roots
     should be traversed when returning the sub-tree (default is false).
    """
    params: T_JSON_DICT = dict()
    params["nodeId"] = node_id.to_json()
    if depth is not None:
...
def set_attributes_as_text(
    ...
    :param text: Text with a number of attributes.
    Will parse this text using HTML parser.
    :param name: *(Optional)*
...
def get_node_stack_traces(
    ...
    T_JSON_DICT, T_JSON_DICT, typing.Optional[runtime.StackTrace]
]:
    """
