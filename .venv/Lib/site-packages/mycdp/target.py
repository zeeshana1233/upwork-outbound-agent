...
def attach_to_target(
    ...
    :param flatten: *(Optional)*
     ...
     in the commands. We plan to make this the default, deprecate non-flattened
...
def close_target(
    ...
    :param target_id:
    :returns: Always set to true.
     If an error occurs, the response indicates protocol error.
    """
    params: T_JSON_DICT = dict()
    params["targetId"] = target_id.to_json()
    cmd_dict: T_JSON_DICT = {
...
def create_browser_context(
    ...
    origins_with_universal_network_access: typing.Optional[
        typing.List[str]
    ] = None,
    ...
    :param dispose_on_detach: **(EXPERIMENTAL)** *(Optional)*
     If specified, disposes this context when debugging session disconnects.
    :param proxy_server: **(EXPERIMENTAL)** *(Optional)*
     Proxy server, similar to the one passed to --proxy-server
    :param proxy_bypass_list: **(EXPERIMENTAL)** *(Optional)*
     Proxy bypass list, similar to the one passed to --proxy-bypass-list
    :param origins_with_universal_network_access: **(EXPERIMENTAL)**
     *(Optional)* An optional list of origins to grant unlimited
     cross-origin access to. Parts of the URL other than those constituting
     origin are ignored.
...
def dispose_browser_context(
    ...
    All the belonging pages will be closed
...
def get_targets(
    ...
    :param filter_: **(EXPERIMENTAL)** *(Optional)*
     Only targets matching filter will be reported.
     If filter is not specified and target discovery is currently enabled,
     a filter used for target discovery is used for consistency.
...
def set_auto_attach(
    ...
    which are considered to be related to this one.
    When turned on, attaches to all existing related targets as well.
    When turned off, automatically detaches from all currently
    attached targets.
    ...
    :param wait_for_debugger_on_start: Whether to pause new targets
     when attaching to them.
     Use ``Runtime.runIfWaitingForDebugger`` to run paused targets.
    :param flatten: **(EXPERIMENTAL)** *(Optional)*
     Enables "flat" access to the session via specifying
     sessionId attribute in the commands. We plan to make this the default,
     deprecate non-flattened mode, and eventually retire it (crbug.com/991325).
    :param filter_: **(EXPERIMENTAL)** *(Optional)*
...
def auto_attach_related(
    ...
    """
    Adds the specified target to the list of targets
    that will be monitored for any related target creation
    (such as child frames, child workers and new versions of service worker)
    and reported through ``attachedToTarget``.
...
def set_discover_targets(
    ...
    :param filter_: **(EXPERIMENTAL)** *(Optional)*
     Only targets matching filter will be attached.
     If ``discover`` is false, ``filter`` must be omitted or empty.
    """
    params: T_JSON_DICT = dict()
