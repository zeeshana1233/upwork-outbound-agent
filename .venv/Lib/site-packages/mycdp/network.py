...
class MonotonicTime(float):
    """Monotonically increasing time (sec) from arbitrary point in the past."""
...
class ResourceTiming:
    ...
    def to_json(self) -> T_JSON_DICT:
        ...
        if self.worker_router_evaluation_start is not None:
            json["workerRouterEvaluationStart"] = (
...
class PostDataEntry:
    ...
    def from_json(cls, json: T_JSON_DICT) -> PostDataEntry:
        return cls(
            bytes_=(
                str(json["bytes"])
                if json.get("bytes", None) is not None
                else None
            ),
        )
...
class Request:
    ...
    #: Note that postData might still be omitted when this flag is true
    ...
    #: Contains the parameters passed by the developer
    #: (e.g. via "fetch") as understood by the backend.
    ...
    def to_json(self) -> T_JSON_DICT:
        ...
        if self.post_data_entries is not None:
            json["postDataEntries"] = [
                i.to_json() for i in self.post_data_entries
            ]
    ...
    def from_json(cls, json: T_JSON_DICT) -> Request:
        return cls(
            ...
            initial_priority=ResourcePriority.from_json(
                json["initialPriority"]
            ),
...
class SecurityDetails:
    ...
    def from_json(cls, json: T_JSON_DICT) -> SecurityDetails:
        return cls(
            ...
            certificate_id=security.CertificateId.from_json(
                json["certificateId"]
            ),
            ...
            certificate_transparency_compliance=CertificateTransparencyCompliance.from_json(  # noqa
                json["certificateTransparencyCompliance"]
            ),
...
class ServiceWorkerRouterInfo:
    ...
    #: The router source of the matched rule. If there is a matched rule,
    #: this field will be set, otherwise no value will be set.
...
class Response:
    ...
    charset: str
    #: Specifies whether physical connection was actually reused
    #: for this request.
    ...
    #: This has been replaced by the headers in
    #: Network.responseReceivedExtraInfo.
    headers_text: typing.Optional[str] = None
    #: Refined HTTP request headers that were actually transmitted
    #: over the network.
    ...
    #: If this field is set with `matchedSourceType` field,
    ...
    service_worker_response_source: typing.Optional[
    ...
    def from_json(cls, json: T_JSON_DICT) -> Response:
        return cls(
            ...
            security_state=security.SecurityState.from_json(
                json["securityState"]
            ),
            headers_text=(
            ...
            service_worker_router_info=(
                ServiceWorkerRouterInfo.from_json(
                    json["serviceWorkerRouterInfo"]
                )
            ...
            alternate_protocol_usage=(
                AlternateProtocolUsage.from_json(
                    json["alternateProtocolUsage"]
                )
...
class WebSocketFrame:
    ...
    #: If the opcode isn't 1,
    #: then payloadData is a base64 encoded string representing binary data.
    payload_data: str
...
class Initiator:
    ...
    #: Initiator URL, set for Parser type or for Script type
    #: (when script is importing module) or for SignedExchange type.
...
class CookiePartitionKey:
    ...
    The representation of the components of the key that are created
    by the cookiePartitionKey class contained in
    net/cookies/cookie_partition_key.h.
    """
    #: The site of the top-level URL the browser was visiting at the start
    #: of the request to the endpoint that set the cookie.
    top_level_site: str
    #: Indicates if the cookie has any ancestors
    #: that are cross-site to the topLevelSite.
...
class Cookie:
    ...
    #: Cookie source port.
    #: Valid values are {-1, [1, 65535]}, -1 indicates an unspecified port.
    #: An unspecified port value allows protocol clients to
    #: emulate legacy cookie scope for the port.
    #: This is a temporary ability and it will be removed in the future.
    source_port: int
...
class CookieExemptionReason(enum.Enum):
    """
    Types of reasons why a cookie should have been blocked by 3PCD,
    but is exempted for the request.
    """
    NONE = "None"
...
class BlockedSetCookieWithReason:
    ...
    #: The string representing this individual cookie
    #: as it would appear in the header.
    #: This is not the entire "cookie" or "set-cookie" header,
    #: which could have multiple cookies.
    cookie_line: str
    #: The cookie object which represents the cookie which was not stored.
    #: It is optional because sometimes complete cookie information
    #: is not available, such as in the case of parsing errors.
    ...
    def from_json(cls, json: T_JSON_DICT) -> BlockedSetCookieWithReason:
        return cls(
            blocked_reasons=[
                SetCookieBlockedReason.from_json(i)
                for i in json["blockedReasons"]
            ],
...
class ExemptedSetCookieWithReason:
    ...
    from a response with the corresponding reason.
    A cookie could only have at most one exemption reason.
    """
    ...
    def from_json(cls, json: T_JSON_DICT) -> ExemptedSetCookieWithReason:
        return cls(
            exemption_reason=CookieExemptionReason.from_json(
                json["exemptionReason"]
            ),
            cookie_line=str(json["cookieLine"]),
...
class AssociatedCookie:
    ...
    #: If empty means the cookie is included.
    ...
    def from_json(cls, json: T_JSON_DICT) -> AssociatedCookie:
        return cls(
            cookie=Cookie.from_json(json["cookie"]),
            blocked_reasons=[
                CookieBlockedReason.from_json(i)
                for i in json["blockedReasons"]
            ],
            exemption_reason=(
...
class CookieParam:
    ...
    #: This value can affect the default domain, path, source port,
    #: and source scheme values of the created cookie.
    url: typing.Optional[str] = None
    #: Cookie domain.
    ...
    #: Cookie source port.
    #: Valid values are {-1, [1, 65535]}, -1 indicates an unspecified port.
    #: An unspecified port value allows protocol clients
    #: to emulate legacy cookie scope for the port.
    #: This is a temporary ability and it will be removed in the future.
    ...
    def from_json(cls, json: T_JSON_DICT) -> CookieParam:
        return cls(
            ...
            domain=(
                str(json["domain"])
                if json.get("domain", None) is not None
                else None
            ),
            path=(
                str(json["path"])
                if json.get("path", None) is not None
                else None
            ),
            secure=(
                bool(json["secure"])
                if json.get("secure", None) is not None
                else None
            ),
            http_only=(
                bool(json["httpOnly"])
...
class SignedExchangeSignature:
    ...
    def from_json(cls, json: T_JSON_DICT) -> SignedExchangeSignature:
        return cls(
            ...
            cert_url=(
                str(json["certUrl"])
                if json.get("certUrl", None) is not None
                else None
            ),
            cert_sha256=(
                str(json["certSha256"])
...
class SignedExchangeHeader:
    ...
    #: Signed exchange header integrity hash in the
    #: form of ``sha256-<base64-hash-value>``.
    ...
    def from_json(cls, json: T_JSON_DICT) -> SignedExchangeHeader:
        return cls(
            ...
            signatures=[
                SignedExchangeSignature.from_json(i)
                for i in json["signatures"]
            ],
...
class ConnectTiming:
    ...
    #: while the other numbers are ticks in
    #: milliseconds relatively to this requestTime.
    #: Matches ResourceTiming's requestTime for the
    #: same request (but not for redirected requests).
    request_time: float
...
class CrossOriginOpenerPolicyStatus:
    ...
    def to_json(self) -> T_JSON_DICT:
        ...
        if self.report_only_reporting_endpoint is not None:
            json["reportOnlyReportingEndpoint"] = (
...
class ReportingApiReport:
    ...
    initiator_url: str
    #: The name of the endpoint group that should be used to
    #: deliver the report.
    destination: str
    #: The type of the report.
    #: (specifies the set of data that is contained in the report body)
    type_: str
    ...
    #: not including an active attempt.
...
def continue_intercepted_request(
    ...
    which either modifies the request to continue with any modifications,
    or blocks it, or completes it with the provided response bytes.
    If a network fetch occurs as a result which encounters a redirect
    an additional Network.requestIntercepted event will be sent with the
    same InterceptionId.
    Deprecated, use Fetch.continueRequest, Fetch.fulfillRequest
    and Fetch.failRequest instead.
    .. deprecated:: 1.3
    **EXPERIMENTAL**
    :param interception_id:
    :param error_reason: *(Optional)*
    ...
    :param raw_response: *(Optional)*
     If set the requests completes using with the provided base64 encoded
     raw response, including HTTP status line and headers etc...
    ...
    :param headers: *(Optional)*
     ...
     Must not be set in response to an authChallenge.
    :param auth_challenge_response: *(Optional)*
...
def delete_cookies(
    ...
    """
    Deletes browser cookies with matching name and url
    or domain/path/partitionKey pair.
    :param name: Name of the cookies to remove.
    :param url: *(Optional)*
     If specified, deletes all the cookies with the given name where domain
     and path match provided URL.
    :param domain: *(Optional)*
     If specified, deletes only cookies with the exact domain.
    :param path: *(Optional)*
     If specified, deletes only cookies with the exact path.
    :param partition_key: **(EXPERIMENTAL)** *(Optional)*
     If specified, deletes only cookies with the the given name
     and partitionKey where all partition key attributes match
...
def emulate_network_conditions(
    ...
    :param latency: Minimum latency from request sent to
     response headers received (ms).
    ...
    :param packet_loss: **(EXPERIMENTAL)** *(Optional)*
     WebRTC packet loss (percent, 0-100).
     0 disables packet loss emulation, 100 drops all the packets.
    :param packet_queue_length: **(EXPERIMENTAL)** *(Optional)*
     WebRTC packet queue length (packet).
     0 removes any queue length limitations.
    :param packet_reordering: **(EXPERIMENTAL)** *(Optional)*
...
def take_response_body_for_interception_as_stream(
    ...
    continued as is -- you either need to cancel it or to provide
...
def search_in_response_body(
    ...
    T_JSON_DICT, T_JSON_DICT, typing.List[debugger.SearchMatch]
]:
    """
...
def set_cache_disabled(
    ...
    If ``true``, cache will not be used.
...
def set_cookie(
    ...
    :param same_party: **(EXPERIMENTAL)** *(Optional)*
     True if cookie is SameParty.
    :param source_scheme: **(EXPERIMENTAL)** *(Optional)*
     Cookie source scheme type.
    :param source_port: **(EXPERIMENTAL)** *(Optional)*
     Cookie source port.
     Valid values are {-1, [1, 65535]}, -1 indicates an unspecified port.
     An unspecified port value allows protocol clients to emulate legacy
     cookie scope for the port. This is a temporary ability
     and it will be removed in the future.
    :param partition_key: **(EXPERIMENTAL)** *(Optional)* Cookie partition key.
     If not set, the cookie will be set as not partitioned.
...
def set_request_interception(
    ...
    .. deprecated:: 1.3
    **EXPERIMENTAL**
    :param patterns: Requests matching any of these patterns will be forwarded
    and wait for the corresponding continueInterceptedRequest call.
    """
    params: T_JSON_DICT = dict()
...
def enable_reporting_api(
    ...
    """
    Enables tracking for the Reporting API.
    Events generated by the Reporting API will now be delivered to the client.
...
def load_network_resource(
    ...
    **EXPERIMENTAL**
    :param frame_id: *(Optional)* Frame id to get the resource for.
...
class DataReceived:
    ...
    #: (might be less than dataLength for compressed encodings).
...
class RequestIntercepted:
    ...
    **EXPERIMENTAL**
    ...
    #: however if any redirects are encountered while processing that fetch,
    #: they will be reported with the same id as the original fetch.
    ...
    #: which can abort the navigation completely.
    is_navigation_request: bool
    #: Set if the request is a navigation that will result in a download.
    #: Only present after response is received from the server
    #: (i.e. HeadersReceived stage).
    ...
    #: event fired for it, then this requestId will be the same as the
...
class RequestWillBeSent:
    ...
    #: Request initiator.
    initiator: Initiator
    #: In the case that redirectResponse is populated,
    #: this flag indicates whether requestWillBeSentExtraInfo
    #: and responseReceivedExtraInfo events will be or were emitted
...
class ResponseReceived:
    ...
    #: Response data.
    response: Response
    #: Indicates whether requestWillBeSentExtraInfo
    #: and responseReceivedExtraInfo events will be
...
class RequestWillBeSentExtraInfo:
    ...
    **EXPERIMENTAL**
    ...
    is available from the network stack. Not every requestWillBeSent event
    will have an additional requestWillBeSentExtraInfo fired for it,
    and there is no guarantee whether requestWillBeSent
    or requestWillBeSentExtraInfo will be fired first for the same request.
    """
    #: Request identifier.
    #: Used to match this information to an existing requestWillBeSent event.
    request_id: RequestId
    #: A list of cookies potentially associated to the requested URL.
    #: This includes both cookies sent with the request and the ones not sent;
    #: the latter are distinguished by having blockedReasons field set.
    ...
    #: Whether the site has partitioned cookies stored
    ...
    def from_json(cls, json: T_JSON_DICT) -> RequestWillBeSentExtraInfo:
        return cls(
            request_id=RequestId.from_json(json["requestId"]),
            associated_cookies=[
                AssociatedCookie.from_json(i)
                for i in json["associatedCookies"]
            ],
            headers=Headers.from_json(json["headers"]),
...
class ResponseReceivedExtraInfo:
    """
    **EXPERIMENTAL**
    Fired when additional information about a responseReceived event
    is available from the network stack. Not every responseReceived event
    ...
    """
    #: Request identifier.
    #: Used to match this information to another responseReceived event.
    request_id: RequestId
    #: A list of cookies which were not stored from the response
    #: along with the corresponding reasons for blocking.
    #: The cookies here may not be valid due to syntax errors,
    #: which are represented by the invalid cookie line string
    ...
    #: The address space can only be determined once the transport
    #: established the connection,
    #: so we can't send it in ``requestWillBeSentExtraInfo``.
    resource_ip_address_space: IPAddressSpace
    #: The status code of the response.
    #: This is useful in cases the request failed and no responseReceived
    #: event is triggered, which is the case for, e.g., CORS errors.
    #: This is also the correct status code for cached requests,
    #: where the status in responseReceived is a 200 and this will be 304.
    status_code: int
    #: Raw response header text as it was received over the wire.
    #: The raw text may not always be available,
    #: such as in the case of HTTP/2 or QUIC.
    ...
    #: True if partitioned cookies are enabled, but the partition key
    #: is not serializable to string.
    cookie_partition_key_opaque: typing.Optional[bool]
    #: A list of cookies which should have been blocked by 3PCD but are
    #: exempted and stored from the response with the corresponding reason.
    ...
    def from_json(cls, json: T_JSON_DICT) -> ResponseReceivedExtraInfo:
        return cls(
            request_id=RequestId.from_json(json["requestId"]),
            blocked_cookies=[
                BlockedSetCookieWithReason.from_json(i)
                for i in json["blockedCookies"]
            ],
            headers=Headers.from_json(json["headers"]),
...
class ResponseReceivedEarlyHints:
    """
    **EXPERIMENTAL**
    Fired when 103 Early Hints headers is received
    in addition to the common response.
    Not every responseReceived event will have
    a responseReceivedEarlyHints fired.
    Only one responseReceivedEarlyHints may be fired for
    each responseReceived event.
    """
    #: Request identifier.
    #: Used to match this information to another responseReceived event.
    request_id: RequestId
...
class SubresourceWebBundleInnerResponseParsed:
    ...
    **EXPERIMENTAL**
    ...
    Note: this will only be fired for resources that are requested
    by the webpage.
    """
...
class SubresourceWebBundleInnerResponseError:
    ...
    **EXPERIMENTAL**
    ...
    #: to another event.
    #: This made be absent in case when the instrumentation was
    #: enabled only after webbundle was parsed.
    bundle_request_id: typing.Optional[RequestId]
    ...
    **EXPERIMENTAL**
    ...
    def from_json(
        ...
        return cls(
            origin=str(json["origin"]),
            endpoints=[
                ReportingApiEndpoint.from_json(i) for i in json["endpoints"]
            ],
        )
