...
class DeepSerializedValue:
    ...
    #: In such case, value is provided only to one of the serialized values.
    #: Unique per value in the scope of one CDP call.
...
class CustomPreview:
    ...
    #: then bodyGetterId will contain RemoteObjectId for the function
    #: that returns result of formatter.body(object, config) call.
...
class PropertyPreview:
    ...
    name: str
    #: Object type.
    #: Accessor means that the property itself is an accessor property.
    type_: str
...
class ExecutionContextDescription:
    ...
    #: Unlike the id, this is unique across multiple processes,
    #: so can be reliably used to identify specific context
...
class StackTrace:
    ...
    #: For async traces this may be a name of the function that
    #: initiated the async call.
    description: typing.Optional[str] = None
    #: Asynchronous JavaScript stack trace that preceded this stack,
    #: if available.
...
def await_promise(
    ...
    :returns: A tuple with the following items:
        0. **result** - Promise result.
         Will contain rejected value if promise was rejected.
        1. **exceptionDetails** - *(Optional)*
...
def call_function_on(
    ...
    :param object_group: *(Optional)*
     ...
     If objectGroup is not specified and objectId is,
     objectGroup will be inherited from object.
    :param throw_on_side_effect: **(EXPERIMENTAL)** *(Optional)*
     Whether to throw an exception if side effect cannot be ruled out
     during evaluation.
    :param unique_context_id: **(EXPERIMENTAL)** *(Optional)*
     An alternative way to specify the execution context to call function on.
     Compared to contextId that may be reused across processes,
...
def evaluate(
    ...
    :param allow_unsafe_eval_blocked_by_csp: **(EXPERIMENTAL)** *(Optional)*
     ...
     when called with non-callable arguments.
     This flag bypasses CSP for this evaluation and allows unsafe-eval.
     Defaults to true.
    :param unique_context_id: **(EXPERIMENTAL)** *(Optional)*
    ...
    if allow_unsafe_eval_blocked_by_csp is not None:
        params["allowUnsafeEvalBlockedByCSP"] = (
...
def get_heap_usage() -> (
    ...
    It is the total usage of the corresponding
...
def get_properties(
    ...
    return (
        ...
        (
            [
                PrivatePropertyDescriptor.from_json(i)
                for i in json["privateProperties"]
            ]
...
def add_binding(
    ...
    this argument should be string, in case of any other input,
    function throws an exception.
    Each binding function call produces Runtime.bindingCalled notification.
    :param name:
    :param execution_context_id: **(DEPRECATED)** **(EXPERIMENTAL)**
    ...
    :param execution_context_name: *(Optional)*
     ...
     See also `ExecutionContext.name` and `worldName` parameters to
